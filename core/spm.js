...
self.start = fibrous( function()
	...
	try {
		...
		if((authenticate && (command == INSTALL || command == SOURCE)) || command == PUBLISH)
			...
			password = read.sync({ prompt: Language.PASSWORD, silent: true });
			}
...
var help = fibrous( function(bVerbose)
	{
	var spm_help = fs.sync.readFile("docs/spm.help", {encoding: "utf8"})
	var spm_parts = spm_help.split(/@verbose/);

	logger.force(spm_parts[0] + (bVerbose ? spm_parts[1] : ""));
	});

var install = fibrous( function(package, username, password, certificate)
	{
	package = Utility.splitPackageName(package);
...
var publish = fibrous( function(package, username, password)
	{
		...
		for(var i=0; i<apps.length; i++)
			...
			if(bVerbose)
				...
				if(apps[i].type == Const.SPACELET)
					...
					path = Config.SANDBOXEDAPPS_PATH;
				else if(apps[i].type == Const.NATIVE_APPLICATION)
					path = Config.NATIVEAPPS_PATH;
