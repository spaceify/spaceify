...
public:
	...
	inline string getFiles(string unique_name, string protocol, sqlite3 *pdb, bool isSpaceify)
	{
		...
		const char *pzTail;
		sqlite3_stmt *pstmt = NULL;
		string injects, str, url_or_path, file_directory, file, inject_type, app_url;

		try
		{
			if(isSpaceify)														// get spaceifys files
				strncpy(sql, "SELECT url_or_path, file_directory, file, inject_type, inject_order FROM inject_files WHERE is_spaceify=1 ORDER BY inject_order ASC", 2048);
			else																// get application files
				sprintf(sql, "SELECT url_or_path, file_directory, file, inject_type, inject_order FROM inject_files WHERE unique_name='%s' ORDER BY inject_order ASC", unique_name.c_str());
			strln = strlen(sql);
			pzTail = sql; pzTail += strln;
			if(sqlite3_prepare(pdb, sql, strln, &pstmt, &pzTail) != SQLITE_OK)
				throw(4);
			...
			while(1)
			{
				...
				file_directory = (char*)sqlite3_column_text(pstmt, 1);
				file = (char*)sqlite3_column_text(pstmt, 2);
				inject_type  = (char*)sqlite3_column_text(pstmt, 3);
				app_url = protocol + "://" + url_or_path + file_directory + file + "?app=" + unique_name + "&type=spacelet";
				...
				if(inject_type == "javascript")
					...
					str = loadFile(url_or_path + file_directory + file);
