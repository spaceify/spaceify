...
#include <time.h>
#include <iostream>
// Get boost from http://www.boost.org/, unpack and copy boost/ to /usr/local/include/
...
public:
	Injector(string path)
	{
		...
		/*// Get values from the Spaceify's configuration file
		boost::property_tree::ptree config;
		boost::property_tree::json_parser::read_json("/var/lib/spaceify/code/www/libs/config.json", config);

		boost::property_tree::ptree &www = config.get_child("WWW_DIRECTORY");
		boost::property_tree::ptree &spa = config.get_child("SPACELETS_PATH");
		boost::property_tree::ptree &vol = config.get_child("VOLUME_DIRECTORY");
		boost::property_tree::ptree &app = config.get_child("APPLICATION_DIRECTORY");

		type_path = spa.get_value<std::string>();
		base_path = vol.get_value<std::string>() + app.get_value<std::string>() + www.get_value<std::string>();*/
	};
	...
	inline string getFiles(string unique_name, string protocol, sqlite3 *pdb, bool isSpaceify)
	{
		...
		{
			/*ifstream bstream("/var/lib/spaceify/code/www/libs/inject/spaceify.csv");
			if(bstream.bad())
				throw(5);
			...
			full_url = protocol + string("://10.0.0.1/libs/spaceifyinitialize.js");			// "://edge.spaceify.net/"
		...
		else																// GET APPLICATION FILES
		{
			try
			{
				...
				while(1)
				{
					...
					full_url = protocol + "://" + url_or_path + "spacelet/" + unique_name + "/get/" + directory + file;
					/*// This is implemented like this also in code/www/libs/spaceify.net (SpaceifyNet::externalResourceURL)
					full_url = protocol + "://" + url_or_path + type_path + unique_name + base_path + directory + file;*/
