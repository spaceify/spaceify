	...
	inline string getFiles(string unique_name, string protocol, sqlite3 *pdb, bool isSpaceify)
	{
		...
		if(isSpaceify)														// GET SPACEIFYS FILES
		{
			...
			while(true)
			{
				...
				{
					full_url = protocol + string("://edge.spaceify.net/") + url;
			...
			full_url = protocol + string("://edge.spaceify.net/libs/spaceifyinitialize.js");
		...
		else																// GET APPLICATION FILES
		{
			try
			{
				...
				while(1)
				{
					...
					full_url = protocol + "://" + url_or_path + "/" + unique_name + "/" + directory + file;
					// This is implemented exactly the same way in SpaceifyNet class (SpaceifyNet::externalResourceURL)
